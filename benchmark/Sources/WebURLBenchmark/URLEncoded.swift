...
let urlEncoded_Decoded = BenchmarkSuite(name: "URLEncoded") { suite in
  ...
  suite.benchmark("String.urlDecoded") {
    for string in urlDecoded_strings {
      blackHole(string.urlDecoded)
    }
  }
  suite.benchmark("String.lazy.urlDecodedScalars iteration") {
    for string in urlDecoded_strings {
      for scalar in string.lazy.urlDecodedScalars {
        blackHole(scalar)
      }
    }
  }
