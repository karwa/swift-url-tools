...
import WebURLTestSupport
...
#if true  // set to 'false' to try using JavascriptCore rather than WKWebView. It might even work!
  ...
  struct JSDOMRunner {
    ...
    func callAsFunction(
      ...
      completionHandler: @escaping (Result<(URLValues), Error>) -> Void
    ) {
      ...
      webview.evaluateJavaScript(js) { maybeResult, maybeError in
        ...
        do {
          guard
            let resultData = try JSONSerialization.jsonObject(
  ...
  struct JSDOMRunner {
    ...
    init() {
      let jsURL = Bundle.main.resourceURL!
      ...
      jsContents = jsContents.replacingOccurrences(
    ...
    func callAsFunction(
      input: String, base: String?, completionHandler: @escaping (Result<(URLValues), Error>) -> Void
    ) {
      ...
      if let result = context.evaluateScript(js), result.isString {
        ...
        do {
          guard
            let resultData = try JSONSerialization.jsonObject(
              with: String(describing: result).data(using: .utf8)!, options: []
            ) as? [String: String]
          else {
            completionHandler(.failure(JSDomRunnerError.deserialisedToUnexpectedDataType))
            return
...
extension JSDOMRunner {
  ...
  /// Executes a series of tests on the JSDOM URL reference implementation, one at a time,
  /// generating a result from each, and delivers the collected results asynchronously as an Array.
  ///
